...
int ES8388::setup(int fs, int channelCount, int bitClkPin, int lrClkPin, int mClkPin,
    ...
    ESP_GOTO_ON_ERROR(AudioDriver::setup(fs, channelCount, bitClkPin, lrClkPin, mClkPin,
    ...
    ret |= WriteReg(ES8388_DACCONTROL3, 0x04);  // 0x04 mute/0x00 unmute&ramp;DAC unmute and  disabled digital volume control soft ramp
...
//    ret |= WriteReg(ES8388_CONTROL2, 0);  //LPVrefBuf=0,Pdn_ana=0
    ...
    ret |= WriteReg(ES8388_DACCONTROL17, 0x90); // only left DAC to left mixer enable 0db
    ret |= WriteReg(ES8388_DACCONTROL20, 0x90); // only right DAC to right mixer enable 0db
    ...
    //ALC for Microphone
    ret |= SetVolume(MODE_ADC, 0, 0);      // 0db
...
bool ES8388::SetI2sSampleRate(I2sSampleRate_t rate)
{
	return false;
}
...
bool ES8388::SetI2sWordSize(I2sWordSize_t size)
{
	return false;
}

bool ES8388::SetI2sClock(I2SClock_t clock)
{
    esp_err_t res = ESP_OK;
    res |= WriteReg(ES8388_MASTERMODE, clock.sclk_div);
    res |= WriteReg(ES8388_ADCCONTROL5, clock.lclk_div);  //ADCFsMode,singel SPEED,RATIO=256
    res |= WriteReg(ES8388_DACCONTROL2, clock.lclk_div);  //ADCFsMode,singel SPEED,RATIO=256
    return (res == ESP_OK);
}
