...
int ES8388::Setup(int fs, int i2c_clk, int i2c_data, int bitClkPin, int lrClkPin, int mClkPin,
                  int dataOutPin, int dataInPin, int enablePin, i2s_port_t i2s_port) {
	esp_err_t ret = ESP_OK;
	this->fs = fs;
    this->i2c_clk = i2c_clk;
	this->i2c_data = i2c_data;
...

	{
		...
		ESP_LOGI(TAG, "I2C SLC = %d, SDA = %d", i2c_clk, i2c_data);
...
bool ES8388::SetGain(Mode_t mode, int volume, int dot) 
{
    int res = 0;
...
bool ES8388::SetVoiceEnabled(bool enable) 
{
    esp_err_t res = ESP_OK;
    uint8_t reg = 0;
...
bool ES8388::GetVoiceEnabled(bool* enable)
{
    esp_err_t res = ESP_OK;
    uint8_t reg = 0;
    res = ReadReg(ES8388_DACCONTROL3, &reg);
...
bool ES8388::SetDACVolume(int volume)
{
    esp_err_t res = ESP_OK;
    if (volume < 0)
        volume = 0;
...
bool ES8388::GetDACVolume(int *volume)
{
    esp_err_t res = ESP_OK;
    uint8_t reg = 0;
